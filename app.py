# app.py  (硬核版：重试 + 退避 + 本地兜底缓存 + 组合 + 盯盘 + 诊断)
import re
import ast
import json
import time
...
def load_portfolio() -> Dict[str, Any]:
    if PORTFOLIO_FILE.exists():
        try:
            return _json_load(PORTFOLIO_FILE)
        except Exception:
            return {}
    return {}


def save_portfolio(cfg: Dict[str, Any]) -> None:
    _json_dump(PORTFOLIO_FILE, cfg)
...
def get_nav(code: str):
    ...
    try:
        ...
        return _parse_nav_tables(r.text)
    except Exception:
        return []
...
with st.sidebar:
    menu = st.radio("菜单", ["搜索", "详情", "组合", "盯盘", "诊断"])
...
st.caption(f"基金列表来源：**{source}**（本地缓存：{FUNDS_CACHE.as_posix()}）")
...
if menu == "详情":
    ...
    if code:
        ...
        with colA:
            ...
            st.caption(f"波动率：{vol:.6f}；最大回撤：{dd*100:.2f}%")
...
# -------------------- 组合 --------------------
if menu == "组合":
    st.subheader("组合（多只基金）")

    cfg = load_portfolio()
    default_codes = "\n".join(cfg.get("codes", ["161725", "008887"]))
    default_up = float(cfg.get("alert_up", 2.0))
    default_dn = float(cfg.get("alert_dn", -2.0))

    codes_text = st.text_area("基金代码（每行一个）", value=default_codes, height=160)
    col1, col2 = st.columns(2)
    with col1:
        alert_up = st.number_input("预警：估值涨幅超过（%）", value=default_up, step=0.1)
    with col2:
        alert_dn = st.number_input("预警：估值跌幅低于（%）", value=default_dn, step=0.1)

    if st.button("保存配置"):
        codes = [c.strip() for c in codes_text.splitlines() if c.strip()]
        save_portfolio({"codes": codes, "alert_up": alert_up, "alert_dn": alert_dn})
        st.success("已保存")
    ...
    for code in codes[:50]:
        ...
        if nav:
            ...
            score, action, vol, dd = (None, "无净值数据", None, None)

        name = gz.get("name") if gz else ""
        gsz = gz.get("gsz") if gz else None
        gszzl = None
        if gz:
            try:
                gszzl = float(gz.get("gszzl"))
            except Exception:
                gszzl = None
        ...
        rows.append({
            ...
            "风险分": score,
            "建议": action,
            "最大回撤%": (dd * 100) if isinstance(dd, float) else None,
        })

    dfp = pd.DataFrame(rows)
    st.dataframe(dfp, use_container_width=True)

    hit_up = dfp[dfp["估值涨跌%"].apply(lambda x: isinstance(x, (int, float)) and x >= alert_up)]
    hit_dn = dfp[dfp["估值涨跌%"].apply(lambda x: isinstance(x, (int, float)) and x <= alert_dn)]

    if len(hit_up) or len(hit_dn):
        st.error("⚠️ 触发预警（见下表）")
        st.dataframe(pd.concat([hit_up, hit_dn]).drop_duplicates(), use_container_width=True)
    else:
        st.success("✅ 暂无触发预警")
...
# -------------------- 盯盘 --------------------
if menu == "盯盘":
    st.subheader("盯盘（页面内自动刷新）")
    st.caption("说明：这是“打开页面就自动更新”的盯盘，不是后台推送。")

    cfg = load_portfolio()
    codes = cfg.get("codes", ["161725"])
    alert_up = float(cfg.get("alert_up", 2.0))
    alert_dn = float(cfg.get("alert_dn", -2.0))

    refresh_sec = st.slider("刷新间隔（秒）", min_value=5, max_value=120, value=15, step=5)
    st.write(f"当前监控：{', '.join(codes)}")
    st.write(f"预警阈值：≥ {alert_up}% 或 ≤ {alert_dn}%")
    ...
    for code in codes[:50]:
        ...
        if gz:
            try:
                ...
                gszzl = None
        rows.append({"代码": code, "名称": name, "估值": gsz, "估值涨跌%": gszzl})

    d = pd.DataFrame(rows)
    st.dataframe(d, use_container_width=True)

    bad = d[d["估值涨跌%"].apply(lambda x: isinstance(x, (int, float)) and (x >= alert_up or x <= alert_dn))]
    if len(bad):
        st.error("⚠️ 有基金触发阈值预警")
...
if menu == "诊断":
    ...
    st.divider()
    st.caption("如果你部署在云上经常失败：通常是云IP被限频。解决路线：降低请求频率/加代理/离线数据。")
